...
export type ImageChoiceMode = 'auto' | 'custom' | 'none';
...
export class TerminalUI {
  ...
  private isAutoGeneratedCard: boolean = false;
  private imageMode: ImageChoiceMode = 'auto';
  ...
  public async start(): Promise<void> {
    ...
    // Escolher modo de imagem no início
    const { imageModeChoice } = await inquirer.prompt([
      {
        type: 'list',
        name: 'imageModeChoice',
        message: 'Como deseja gerenciar a imagem/banner da postagem?',
        choices: [
          { name: '🎨 Gerar Banner Visual com IA (Automático em PNG)', value: 'auto' },
          { name: '📁 Anexar uma imagem própria do computador', value: 'custom' },
          { name: '🚫 Não incluir imagem (Post apenas em texto)', value: 'none' }
        ]
      }
    ]);

    this.imageMode = imageModeChoice;

    if (this.imageMode === 'custom') {
      await this.promptCustomImage();
    }
  ...
  private async generatePost(sanitizedContext: any, sanitizedNotes: string, userInstruction?: string): Promise<void> {
    ...
    try {
      ...
      // Gerenciar a imagem conforme a escolha do usuário
      if (this.imageMode === 'auto') {
        const spinnerCard = ora('Gerando Card Visual dinâmico em PNG (1200x630px)...').start();
        try {
          ...
          this.isAutoGeneratedCard = true;
          spinnerCard.succeed(`Card Visual gerado com sucesso: ${chalk.bold(cardPath)}\n`);
        } catch {
          spinnerCard.warn('Não foi possível gerar o Banner Visual. O post prosseguirá apenas com texto.\n');
          this.lastGeneratedCardPath = undefined;
          this.isAutoGeneratedCard = false;
        }
      } else if (this.imageMode === 'none') {
        this.lastGeneratedCardPath = undefined;
        this.isAutoGeneratedCard = false;
      }
  ...
  private displayPost(postText: string): void {
    ...
    if (this.lastGeneratedCardPath) {
      const typeLabel = this.isAutoGeneratedCard ? 'CARD VISUAL DA IA' : 'IMAGEM PRÓPRIA ANEXADA';
      console.log(chalk.bold.cyan(`🖼️  ${typeLabel}: ${this.lastGeneratedCardPath}`));
    } else {
      console.log(chalk.bold.gray('📝 MODO DE IMAGEM: Apenas Texto (Sem Imagem)'));
    }
    console.log(chalk.bold.green('----------------------------------------------------\n'));
  }
  ...
  private async showOptionsMenu(postText: string, sanitizedContext: any, sanitizedNotes: string): Promise<void> {
    const { action } = await inquirer.prompt([
      {
        ...
        choices: [
          ...
          { name: '🖼️  Alterar Imagem / Banner (Própria, IA ou Remover)', value: 'change_image' },
          { name: '🚀 Publicar no LinkedIn', value: 'publish' },
    ...
    switch (action) {
      case 'save': {
        ...
        if (this.lastGeneratedCardPath) {
          console.log(chalk.cyan(`🖼️  Imagem/Card salvo em: ${chalk.bold(this.lastGeneratedCardPath)}\n`));
        }
        await this.showOptionsMenu(postText, sanitizedContext, sanitizedNotes);
        break;
      }
      ...
      case 'change_image': {
        const { newImageChoice } = await inquirer.prompt([
          {
            type: 'list',
            name: 'newImageChoice',
            message: 'Escolha a opção de imagem para a postagem:',
            choices: [
              { name: '🎨 Gerar um Novo Banner Visual com a IA', value: 'auto' },
              { name: '📁 Escolher uma imagem do computador', value: 'custom' },
              { name: '🚫 Remover imagem (Post apenas em texto)', value: 'none' }
            ]
          }
        ]);

        this.imageMode = newImageChoice;

        if (this.imageMode === 'custom') {
          await this.promptCustomImage();
        } else if (this.imageMode === 'auto') {
          if (this.isAutoGeneratedCard) this.cleanupTempCard();
          const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
          const cardFilename = `card_${sanitizedContext.repoName}_${timestamp}.png`;
          const cardPath = path.join(process.cwd(), '.posts', cardFilename);
          ...
          this.isAutoGeneratedCard = true;
          console.log(chalk.green(`\n🎨 Novo Banner Visual gerado em: ${chalk.bold(this.lastGeneratedCardPath)}\n`));
        } else if (this.imageMode === 'none') {
          if (this.isAutoGeneratedCard) this.cleanupTempCard();
          this.lastGeneratedCardPath = undefined;
          this.isAutoGeneratedCard = false;
          console.log(chalk.yellow('\n🚫 Imagem removida da postagem.\n'));
        }
  ...
  private async promptCustomImage(): Promise<void> {
    const { customPath } = await inquirer.prompt([
      {
        type: 'input',
        name: 'customPath',
        message: 'Digite o caminho da imagem no computador (ex: ./minha_foto.png ou /home/user/print.jpg):'
      }
    ]);
    ...
    if (customPath && customPath.trim()) {
      ...
      if (fs.existsSync(resolvedPath)) {
        if (this.isAutoGeneratedCard) this.cleanupTempCard();
        this.lastGeneratedCardPath = resolvedPath;
        this.isAutoGeneratedCard = false;
        console.log(chalk.green(`\n✅ Imagem própria selecionada: ${chalk.bold(resolvedPath)}\n`));
      } else {
        console.log(chalk.red(`\n❌ Arquivo de imagem não encontrado em: ${resolvedPath}`));
        console.log(chalk.yellow('Mantendo modo sem imagem por enquanto.\n'));
        this.lastGeneratedCardPath = undefined;
        this.isAutoGeneratedCard = false;
      }
    }
  ...
  private cleanupTempCard(): void {
    if (this.lastGeneratedCardPath && this.isAutoGeneratedCard && fs.existsSync(this.lastGeneratedCardPath)) {
      try {
